...
PROTOCOL CRACKER
================
Discover the sensor's complete custom command set
"""
...
from CustomProtocolAnalyzer import CustomProtocolAnalyzer, Colors
...
async def main():
    ...
    print("╔═══════════════════════════════════════════════════════════════════════════════╗")
    print("║                    CUSTOM PROTOCOL CRACKER v1.0                               ║")
    print("║                    Brute Force Command Discovery                              ║")
    print("║                                                                               ║")
    print("║  Discovers:                                                                   ║")
    print("║  • All working custom commands                                                ║")
    print("║  • Response patterns and data types                                           ║")
    print("║  • Configuration interface                                                    ║")
    print("║  • Server/network commands                                                    ║")
    print("║  • Firmware/debug commands                                                    ║")
    print("╚═══════════════════════════════════════════════════════════════════════════════╝")
    print(f"{Colors.RESET}")
    
    print(f"\n{Colors.YELLOW}⚠️  This will test 200+ command variations{Colors.RESET}")
    print(f"{Colors.YELLOW}   Analysis time: ~5-10 minutes{Colors.RESET}")
    ...
    async with BleakClient(device.address, timeout=30.0) as client:
        ...
        # Create analyzer
        analyzer = CustomProtocolAnalyzer(client, write_char, notify_char)
        
        # Run protocol analysis
        print(f"\n{Colors.GREEN}[STARTING] Protocol analysis...{Colors.RESET}")
        print(f"{Colors.DIM}This will take several minutes. Please wait...{Colors.RESET}\n")
        
        commands, analysis, report_file = await analyzer.run_protocol_analysis()
        
        print(f"\n{Colors.GREEN}{'='*80}{Colors.RESET}")
        print(f"{Colors.GREEN}[COMPLETE] Protocol analysis finished!{Colors.RESET}")
        print(f"{Colors.GREEN}{'='*80}{Colors.RESET}\n")
        
        if report_file:
            print(f"{Colors.CYAN}Report saved to:{Colors.RESET} {report_file}")
            print(f"\n{Colors.YELLOW}Next steps:{Colors.RESET}")
            print(f"  1. Review protocol_map_*.json for all discovered commands")
            print(f"  2. Look for configuration/server commands")
            print(f"  3. Test commands to change settings")
            print(f"  4. Build custom firmware if needed")
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n{Colors.YELLOW}[EXIT] Analysis interrupted{Colors.RESET}")
